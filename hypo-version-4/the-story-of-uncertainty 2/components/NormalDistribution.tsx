      ...
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-auto max-h-full drop-shadow-sm overflow-visible">
        ...
        {showCentralRegion && (
          <g className="animate-in fade-in duration-700">
            ...
            <text x={xScale(mean)} y={yScale(pdf(mean)) + 40} textAnchor="middle" fontSize="24" fontWeight="900" fill="#1e40af" className="drop-shadow-sm">
              {((1 - alpha) * 100).toFixed(0)}%
            </text>
            <text x={xScale(mean)} y={yScale(pdf(mean)) + 60} textAnchor="middle" fontSize="10" fontWeight="900" fill="#2563eb" className="uppercase tracking-[0.2em]">Expected Region</text>
            ...
            <g transform={`translate(0, ${height - margin.bottom + 45})`}>
        ...
        {showCriticalRegion && (
          <g className="animate-in fade-in duration-500">
            ...
            <g transform={`translate(0, ${height - margin.bottom + 50})`}>
               {isTwoTailed && (
                 <g>
                    <line x1={xScale(criticalXLeft) - 5} y1="0" x2={xScale(xRange[0]) + 10} y2="0" stroke="#ef4444" strokeWidth="2" markerEnd="url(#arrowhead-red-left)" />
                    <text x={xScale(criticalXLeft) - 10} y="-10" textAnchor="end" fontSize="9" fontWeight="900" fill="#ef4444" className="uppercase tracking-[0.1em]">Reject H₀</text>
                 </g>
               )}
               <g>
                  <line x1={xScale(criticalXRight) + 5} y1="0" x2={xScale(xRange[1]) - 10} y2="0" stroke="#ef4444" strokeWidth="2" markerEnd="url(#arrowhead-red-right)" />
                  <text x={xScale(criticalXRight) + 10} y="-10" textAnchor="start" fontSize="9" fontWeight="900" fill="#ef4444" className="uppercase tracking-[0.1em]">Reject H₀</text>
               </g>
               <g>
                  <line 
