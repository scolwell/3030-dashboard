          ...
          <div className="flex flex-col items-center justify-center gap-8 h-full py-4">
            ...
            <div className="text-center bg-slate-50 px-6 py-4 rounded-2xl border border-slate-100 max-w-sm">
                ...
                <p className="text-slate-400 text-xs mt-1 uppercase font-bold tracking-tighter">
                  Is this noise or evidence of real change?
                </p>
            </div>
          </div>
        );
          ...
          <div className="flex flex-col items-center justify-center w-full animate-in fade-in duration-700 py-4 max-w-2xl h-full">
            <div className="grid grid-cols-3 grid-rows-3 gap-3 w-full">
              ...
              <div className="bg-red-50 border-2 border-red-100 p-4 rounded-2xl flex flex-col items-center text-center">
                ...
                <p className="text-[9px] text-red-600 mt-1">Rate = α</p>
              </div>
    ...
    <div className="min-h-screen flex flex-col items-center py-16 px-4 sm:px-8 lg:px-12 bg-slate-50">
      <header className="max-w-7xl w-full flex flex-col md:flex-row items-center justify-between mb-16 gap-8">
        <div className="text-center md:text-left">
          ...
          <p className="text-slate-500 font-semibold tracking-wide uppercase text-[11px]">Philosophy & Logic of Hypothesis Testing</p>
        </div>
      </header>
        ...
        <div className="lg:col-span-5">
          <div className="bg-white rounded-[3rem] shadow-2xl shadow-slate-200/50 border border-slate-100 p-12 h-full flex flex-col relative overflow-hidden">
            ...
            <div key={state.currentStep} className="animate-in fade-in slide-in-from-bottom-4 duration-700">
              <span className="inline-block px-4 py-1 rounded-full bg-amber-50 text-amber-700 text-[10px] font-black uppercase tracking-widest mb-4 border border-amber-200">📖 Textbook Ch. {step.chapterNumber}</span>
              ...
              {ASSESSMENT_QUESTIONS[state.currentStep] && (
                <div className="mb-8">
                  <AssessmentCheckpoint 
                    stepId={state.currentStep} 
                    question={ASSESSMENT_QUESTIONS[state.currentStep]} 
            logic':
        return (
          <div className="flex flex-col items-center justify-center gap-6 py-4 animate-in fade-in zoom-in duration-500 w-full max-w-lg h-full">
            <div className="grid grid-cols-2 gap-4 w-full">
      ...
      case 'errors':
        return (
          <div className="flex flex-col items-center justify-center w-full animate-in fade-in duration-700 py-4 max-w-2xl h-full">
            <div className="grid grid-cols-3 grid-rows-3 gap-3 w-full">
              ...
              <div className="row-start-3 text-right text-[10px] font-black uppercase text-slate-400 flex items-center justify-end pr-2">Decision: STAY</div>
              <div className="bg-slate-50 border-2 border-slate-100 p-blue-50 text-blue-600 text-[10px] font-black uppercase tracking-widest mb-4">Chapter
                <p className="text-slate-900 font-black text-xs uppercase mb-1">Correct</p>
                <p className="text-[10px] text-slate-700">Safety</p>
              </div>
              <div className="bg-amber-50 border-2 border-amber-100 p-4 rounded-2xl flex flex-col items-center text-center">
                <p className="text-amber-900 font-black text-xs uppercase mb-1">Type II Error</p>
                <p c<div className="flex flex-col items-center gap-4">
                   <div className="bg-white border-2 border-red-500 text-red-500 w-16 h-16 rounded-full flex items-center justify-center text-2xl shadow-lg rotate-12">
                      <i className="fas fa-times"></i>
                   </div>
                   <span className="text-xs font-black text-slate-400 uppercase tracking-widest">Reject H₀</span>
                ...
                <div className="flex flex-col items-center gap-4">
                  ...
                  <div className="mt-8 bg-amber-50 border-l-4 border-amber-500 p-8 rounded-r-[2rem] shadow-xl animate-in slide-in-from-bottom-2 duration-700">
                    <div className="flex items-center gap-3 mb-3">
                      <i className="fas fa-landmark text-amber-500 text-xl"></i>
                      <h5 className="text-amber-900 font-black text-xs uppercase tracking-widest">Scientific Convention</h5>
                    </div>
                    <p className="text-amber-800 text-base leading-relaxed font-semibold italic">
                  ...
                  <div className="mt-8 bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10">
                      <h5 className="text-blue-400 font-black text-xs uppercase tracking-[0.2em] mb-3">Alpha Rule</h5>
                      <p className="text-sm text-slate-300 leading-relaxed font-medium">
                        Choosing <span className="text-white font-bold">&alpha; = {state.alpha}</span> is a commitment to being wrong no more than {state.alpha * 100}% of the time when claiming a discovery that doesn't exis
                ) : (
                  <div className="mt-8 bg-slate-900 text-white p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                    <div className="absolute top-0 right-0 p-4 opacity-10 rotate-12 transition-transform group-hover:rotate-45">
                      <i className="fas fa-microscope text-6xl"></i>
                    ...
                    <div className="relative z-10">
                      <h5 className="text-blue-400 font-black text-xs uppercase tracking-[0.2em] mb-3">Decision Rule</h5>
                      <p className="text-sm text-slate-300 leading-relaxed font-medium">
                        If p-value &lt; <span className="text-white font-bold">&alpha; = {state.alpha}</span>, we reject H₀ and claim a statistically significant result.
                      </p>
                    </div>
                  </div>
                )
        ...
        <div className="lg:col-span-7">
          <div className="bg-white rounded-[3rem] shadow-2xl shadow-slate-200/50 border border-slate-100 p-8 h-full flex flex-col items-center justify-center relative overflow-hidden min-h-[600px]">
            ...
            {state.currentStep >= 5 && state.currentStep < STORY_STEPS.length - 1 && (
                    ...
                    {state.currentStep >= 7 && (
                      <div className="w-full sm:w-48 text-center border-t sm:border-t-0 sm:border-l border-slate-200 pt-6 sm:pt-0 sm:pl-10">
                          ...
                          <div className="text-[9px] text-slate-500 mt-2 font-bold">
                            {pValue < state.alpha ? '✓ Reject H₀' : 'Fail to Reject H₀'}
                          </div>
                      </div>
                    )}
                </div>
